...
def entry_point_picard1(pg_conn,project_code, pipeline_id,task_id,next_task_id,mount_point,run_dry=True,resetquery=True,one_sample=True):
...
##queries
    ...
    while(len(samples) > 0):
        ...
        update_status_sample = update_status_sample_tmp % ('running', 'picard1', sample_id, project_code)
        ...
        query_update = query_update_next_step % (next_table_name,dir_output_db, filename_output,'null','null','pending', date,run_time_next,trimmed_quality,pipeline_id,next_task_id,sample_id,filename_output)
        if(run_dry):
            print(query_insert)
            print(query_update)
        else:
