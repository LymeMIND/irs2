...
##queries
    ...
    query_insert_next_step = 'INSERT INTO %s (pipeline_id,task_id,sample_id,dir_input,filename_input,dir_output,filename_output,status,date,run_time, trimmed_quality,file_extension)'\
    ' VALUES(%d,%d,\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',%f,%d , \'bai\')'
    query_update_next_step = 'UPDATE %s SET dir_input=\'%s\', filename_input=\'%s\', dir_output=\'%s\', filename_output=\'%s\','\
            ' status=\'%s\' ,date=\'%s\',run_time=%f, trimmed_quality=%d WHERE filename_input=\'%s\'AND pipeline_id=%d AND task_id=%d AND sample_id=\'%s\''


    option_cmd = 'BuildBamIndex'
    ...
    #
    suffix = 'Aligned.sortedByCoord.WithReadGroups.bai'
    while(len(samples) > 0):
        ...
        #TO CHECK IF CAN MERGE PICARD1 and PICARD2
        options_change  = 'I={input} O={output}'.format(input=samplefile,output=full_filename_output)
...
if __name__ == '__main__':
    ...
    entry_point_picard2(pg_conn,project,pipeline_id,task_id,next_task_id,run_dry=False,resetquery=True,one_sample=True)
